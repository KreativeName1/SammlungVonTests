      ...
      <?php
        if (isset($_POST['change_profile']) && $_SESSION['user'] == $id) {
          ...
          foreach ($users as $user) {
            if ($user['id'] == $url[0]) {
              $userC ->updateUser($url[0], 'username', $name);
              $userC ->updateUser($url[0], 'bio', $bio);
              if ($image != "") {
                move_uploaded_file($_FILES['profilePicture']['tmp_name'], "images/$image");
                $user ->updateUser($id, "image", $image);
              }
            }
          }
          // file_put_contents("users.json", json_encode($userC, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE), LOCK_EX);
        }
      ?>
    </div>
  </body>
